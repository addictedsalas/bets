...
import { OddsService, Game } from './oddsService';
...
export class GameMonitor {
  ...
  async checkGames(): Promise<void> {
    try {
      console.log('🔍 Checking for basketball games...');
      
      const liveGames = await this.oddsService.getLiveBasketballGames();
      const targetGames = liveGames.filter(game => 
        this.oddsService.isThirdQuarterSevenMinutes(game)
      );
  ...
  private async analyzeGame(game: Game): Promise<void> {
    try {
  ...
  private async sendTelegramNotification(opportunity: BetOpportunity): Promise<void> {
    try {
...
🎲 Place this bet on FanDuel!
  ...
  private cleanupProcessedGames(): void {
